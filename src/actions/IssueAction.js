...
export const getUserIssuedBook = () => async (dispatch, getState) => {
    ...
    try {
        const response = await axios.get(`http://localhost:5000/api/issues/issuedBook`, config);
        dispatch({
...
export const issuedReq = (bookId,postId)=> async dispatch =>{
    try {
         await axios.post('http://localhost:5000/api/issues/issuedReqAccept' , {bookId,postId})
...
export const issueABookReturn = (postId)=> async dispatch =>{
    ...
    try {
        ...
        const response2 = await axios.get(`http://localhost:5000/api/issues/issuedBook`, config);
